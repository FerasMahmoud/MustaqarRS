    ...
    <div className="min-h-screen bg-cream pt-24 pb-16">
      ...
      <div className="container-luxury py-12">
        <div className="max-w-4xl mx-auto">
          {/* Important Notice */}
          <div
            className={`bg-gold/10 border border-gold/30 rounded-xl p-6 mb-10 ${isRtl ? 'text-right' : 'text-left'}`}
            role="alert"
          >
          ...
          {/* Quick Navigation */}
          <nav
            className={`bg-white rounded-xl p-6 shadow-sm mb-10 ${isRtl ? 'text-right' : 'text-left'}`}
          ...
          <div className="space-y-10">
            {sections.map((section) => {
              ...
              const items = section.listKey
                ? (t.raw(`sections.${section.key}.${section.listKey}`) as string[])
                : null;
              ...
              return (
                <article
                  key={section.key}
                  id={section.key}
                  className={`bg-white rounded-xl p-8 shadow-sm scroll-mt-24 ${isRtl ? 'text-right' : 'text-left'}`}
                >
                  <div className={`flex items-center gap-4 mb-6 ${isRtl ? 'flex-row-reverse' : ''}`}>
                    <div
                      className="w-12 h-12 bg-gold/10 rounded-lg flex items-center justify-center flex-shrink-0"
                      aria-hidden="true"
                    >
                  ...
                  {items && items.length > 0 && (
                    <ul className={`space-y-3 mt-4 ${isRtl ? 'pr-4' : 'pl-4'}`}>
                      {items.map((item: string, index: number) => (
                        ...
                        >
                          <span
                            className="w-1.5 h-1.5 bg-gold rounded-full mt-2.5 flex-shrink-0"
                            aria-hidden="true"
                          ></span>
                          <span>{item}</span>
                        </li>
            ...
            <article
              id="contact"
              className={`bg-charcoal text-white rounded-xl p-8 scroll-mt-24 ${isRtl ? 'text-right' : 'text-left'}`}
            >
              <div className={`flex items-center gap-4 mb-6 ${isRtl ? 'flex-row-reverse' : ''}`}>
                <div
                  className="w-12 h-12 bg-gold/20 rounded-lg flex items-center justify-center flex-shrink-0"
                  aria-hidden="true"
                >
                  <Mail className="w-6 h-6 text-gold" />
                </div>
                <h2 className="text-xl md:text-2xl font-semibold">{t('sections.contact.title')}</h2>
              </div>
              <p className="text-white/70 leading-relaxed mb-6">{t('sections.contact.content')}</p>
          ...
          {/* Footer Navigation */}
          <nav
            className={`mt-12 pt-8 border-t border-border flex flex-col sm:flex-row justify-between items-center gap-4 ${isRtl ? 'sm:flex-row-reverse' : ''}`}
            aria-label={isRtl ? 'روابط الصفحة' : 'Page navigation'}
          >
            <Link
              href={`/${locale}`}
              className={`flex items-center gap-2 text-charcoal-light hover:text-gold transition-colors ${isRtl ? 'flex-row-reverse' : ''}`}
